...
> [!NOTE]
> Para una perspectiva estratégica de alto nivel sobre los proveedores de nube, estimación de costos mensuales, topología de red y cumplimiento de seguridad, consulte la [Documentación Ejecutiva de Despliegue](file:///c:/Users/quisp/Documents/GitHub/fullstack-backend/DOCUMENTACION_EJECUTIVA_DESPLIEGUE.md).
...
3.  **Alternativa en la Nube: Despliegue de la Base de Datos usando Aiven:**
    Aiven es una excelente plataforma para alojar bases de datos gestionadas de forma rápida (incluyendo un plan gratuito para MySQL). Sigue estos pasos para desplegar la base de datos en Aiven:

    *   **Paso A: Crear el servicio de MySQL en Aiven**
        1. Regístrate o inicia sesión en [Aiven.io](https://aiven.io/).
        2. Crea un nuevo proyecto y luego haz clic en **Create Service**.
        3. Selecciona **MySQL** como el tipo de servicio.
        4. Elige un proveedor de nube (por ejemplo, AWS o GCP), la región geográfica más cercana y selecciona el plan de servicio (el plan gratuito es suficiente para desarrollo y pruebas).
        5. Asigna un nombre al servicio y haz clic en **Create Service**. Espera unos minutos a que el estado del servicio pase de *Rebuilding* a *Running*.

    *   **Paso B: Obtener las credenciales de conexión**
        Una vez que el servicio esté activo, en la pestaña *Overview* verás la información de conexión. Toma nota de los siguientes valores:
        *   **Host**: El hostname de conexión (ej. `mysql-tu-proyecto.aivencloud.com`).
        *   **Port**: El puerto asignado (ej. `23456`).
        *   **User**: Por defecto es `avnadmin`.
        *   **Password**: La contraseña segura autogenerada por Aiven.

    *   **Paso C: Cargar el esquema (`bd.sql`) con SSL obligatorio**
        Aiven requiere que todas las conexiones a MySQL sean cifradas mediante SSL. Para importar el esquema local [bd.sql](file:///c:/Users/quisp/Documents/GitHub/fullstack-backend/bd.sql) a tu base de datos remota, ejecuta el siguiente comando en tu terminal local (reemplazando los marcadores por tus datos reales):
        ```bash
        mysql -h <TU_HOST_DE_AIVEN> -P <TU_PUERTO_DE_AIVEN> -u avnadmin -p --ssl-mode=REQUIRED < bd.sql
        ```
        > [!NOTE]
        > Si prefieres usar una herramienta de interfaz gráfica como **DBeaver** o **MySQL Workbench**, crea una nueva conexión con los datos de Aiven y asegúrate de habilitar la opción de **SSL** (usualmente configurado como *Require* o *Verify CA*) antes de ejecutar el archivo SQL.
        > El script `bd.sql` se encargará de crear y poblar la base de datos `db_visitas_inopinadas` dentro de tu instancia de Aiven.

    *   **Paso D: Configuración en el Backend**
        Cuando configures el backend para conectarse a Aiven (ya sea en local o en Render/Railway), proporciona los siguientes valores en las variables de entorno:
        *   `DB_HOST`: El host de tu base de datos en Aiven.
        *   `DB_PORT`: El puerto de tu base de datos en Aiven.
        *   `DB_NAME`: `db_visitas_inopinadas` (nombre de la base de datos que se crea al ejecutar `bd.sql`).
        *   `DB_USER`: `avnadmin`.
        *   `DB_PASSWORD`: La contraseña de tu servicio de Aiven.
        
        *Nota: La configuración por defecto en `application.properties` (`useSSL=true`) se encargará de asegurar que la conexión de Spring Boot a la base de datos en Aiven esté debidamente encriptada.*
